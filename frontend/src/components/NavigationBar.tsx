  ...
  return (
    <>
      ...
      <nav className="bg-[#ff8a65] p-4 flex justify-between items-center">
        <div className="text-white text-2xl font-bold">Digicap</div>
        ...
        <div className="hidden md:flex items-center gap-10">
          ...
          {isAuthenticated ? (
            <button
              ...
              className="bg-white text-[#ff8a65] px-5 py-2 rounded-lg cursor-pointer font-semibold shadow-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-orange-400"
            >
              Logout
            </button>
          ) : (
            ...
            <div className="fixed top-0 right-0 w-3/4 h-full bg-white z-50 shadow-lg transition-transform transform duration-300 ease-in-out">
              <div className="p-6">
                ...
                {isAuthenticated ? (
                  <button
                    onClick={handleLogout}
                    className="block px-4 py-2 text-center w-full cursor-pointer text-white bg-[#ff8a65] font-semibold mt-4"
                  >
                    Logout
                  </button>
                ) : (
                  <NavLink
                    to="/register"
                    className="block px-4 py-2 text-center cursor-pointer text-white bg-[#ff8a65] font-semibold mt-4"
                  >
                    Sign in
                  </NavLink>
                )}
                <div className="mt-6 border-t border-gray-200 pt-6 text-center text-gray-500">
                  &copy; {new Date().getFullYear()} Digicap. All rights
                  reserved.
                </div>
              </div>
            </div>
